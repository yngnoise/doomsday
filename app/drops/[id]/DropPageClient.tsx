...
import type { DropData, DropPhase, SizeInfo } from "@/lib/dropApi";
...
function useDropData(dropID: string, initialData: DropData | null) {
  const [data,    setData]    = useState<DropData | null>(initialData);
...
export default function DropPageClient({
  ...
  const { data, loading, error } = useDropData(DROP_ID, initialData);
