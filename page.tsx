...
// ─────────────────────────────────────────────────────────────────────────────
const PHOTO_COUNT = 4; // change to match how many photos you have
const PHOTOS = Array.from({ length: PHOTO_COUNT }, (_, i) => `/product/${i + 1}.jpg`);
...
// ─────────────────────────────────────────────────────────────────────────────
function PhotoViewer({ phase, price, edition }: {
  phase: DropPhase; price: string; edition: string;
}) {
...
// ─────────────────────────────────────────────────────────────────────────────
function ReservationOverlay({ expiresAt, reservationID }: { expiresAt: string; reservationID: string }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
      ...
      <button onClick={() => router.push(`/checkout/${reservationID}?expires=${encodeURIComponent(expiresAt)}`)}
...
// ROOT
// ─────────────────────────────────────────────────────────────────────────────
const DROP_ID = "dmsdy-ss25-001";
const ITEM_ID = "jacket-001";
const SPECS   = ["Waxed Ventile® Shell","YKK® Aquaguard Zip","D-ring Utility"];
const EDITION = "SS/25 — COLLECTION 001";

export default function DoomsdayDrop() {
    ...
    <div className="h-screen w-full bg-black text-white flex flex-col overflow-hidden"
      ...
      <header className="relative z-10 flex-shrink-0 flex items-center justify-between px-6 py-4 border-b border-zinc-800">
        <div className="flex items-center gap-4">
          ...
          <span className="hidden sm:block text-xs font-mono tracking-widest uppercase text-zinc-500">DROP #001</span>
        </div>
        <div className="flex items-center gap-4">
          {phase === "live" && (
      ...
      <main className="relative z-10 flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-[1fr_480px]">
        ...
        <div className="hidden lg:block border-r border-zinc-800 h-full">
          <PhotoViewer phase={effectivePhase} price={price} edition={EDITION} />
        </div>
